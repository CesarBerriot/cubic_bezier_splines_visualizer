...
{	for(int i = 0; i < LAYER_MAX; ++i)
	{   sfRenderTexture * layer = sfRenderTexture_create(sfRenderWindow_getSize(window), &context_settings);
...
{	sfView * window_view_backup = sfView_copy(sfRenderWindow_getView(window));
	...
	{   sfRenderTexture * layer = layers[i];
		...
		sfSprite * sprite = sfSprite_create(sfRenderTexture_getTexture(layer));

		// SFML bug workaround
		sfRenderTexture * throwaway_render_texture = sfRenderTexture_create((sfVector2u){ .x = 1, .y = 1 }, NULL);
