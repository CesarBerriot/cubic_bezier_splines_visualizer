		...
		{	case sfEvtMouseMoved:
				{	static struct
					...
					{	float sensitivity = .001;
						...
						window_view_data.position.x += (event.mouseMove.position.x - last.event.position.x) * scale_factor;
						window_view_data.position.y += (event.mouseMove.position.y - last.event.position.y) * scale_factor;
